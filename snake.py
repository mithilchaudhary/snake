...
class SNAKE:
    ...
    def move_snake(self):
        ...
        if self.body[0].x > cell_number or self.body[0].x < -1:
            if self.body[0].x >= cell_number:
                self.body[0].x = 0
            else:
                self.body[0].x = cell_number-1
        if self.body[0].y > cell_number or self.body[0].y < -1:
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == SCREEN_UPDATE:
            if(not main_game.pause):
                main_game.update()

        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_SPACE:
                main_game.pause = not main_game.pause
